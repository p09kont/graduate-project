...
if (isset($_GET["id"])) {
    ...
    if(isset($_POST["confId"])){
        $thisConferenceId = $_POST["confId"];
        $allConfernceArticles = "SELECT p.id, p.title, p.year, p.page_start, p.page_end, p.cited_by, c.name AS conference, c.date, c.location, c.volume, pr.name AS proccendings 
                                    FROM publications p 
                                    INNER JOIN conference_articles ca on ca.pub_id = p.id
                                    INNER JOIN conferences c ON c.id = ca.conf_id
                                    INNER JOIN proccendings pr ON pr.id = c.proc_id
                                    WHERE ca.conf_id = $thisConferenceId";
    }
    else{
        $allConfernceArticles = "SELECT p.id, p.title, p.year, p.cited_by, p.page_start, p.page_end, c.name AS conference, c.date, c.location, c.volume, pr.name AS proccendings   
                            FROM publications p
